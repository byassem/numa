...
function VerifyEmailCodeSignUpForm({
  ...
  const headerChildren: ReactNode = (
    ...
    >
      <Text style={styles.emailAddressText}>
        {emailAddress}{" "}
        <Ionicons name="pencil-outline" size={16} color="#5e41f7" />
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    ...
    >
      <OTPInput value={code} onChange={(code) => setCode(code)} />
      ...
      >
        <Text
          style={[styles.switchModeText, !canResend && styles.disabledText]}
        >
          Didn&apos;t receive a code?{" "}
          {canResend ? "Resend" : `Resend (${resendTimer})`}
        </Text>
      </TouchableOpacity>
